...
def banner(style: Style) -> str:
    title = style.bold(style.bright_cyan("disktidy"))
    ver = style.dim(f"v{__version__}")
...
def bar(style: Style, frac: float, width: int = 14) -> str:
    ...
    color = style.red if frac >= 0.9 else style.yellow if frac >= 0.8 else style.green
    return color(body)


def size_color(style: Style, nbytes: int, text: str) -> str:
    """Color a size string by magnitude: red >=10G, yellow >=1G, else green."""
    if nbytes >= 10 * 1024**3:
...
def table(
    ...
    def rule(left: str, mid: str, right: str) -> str:
        return style.gray(left + mid.join("─" * (w + 2) for w in widths) + right)

    def pad(text: str, i: int) -> str:
        return text.rjust(widths[i]) if aligns[i] == "r" else text.ljust(widths[i])

    lines = [rule("┌", "┬", "┐")]
    header_cells = [style.bold(pad(str(h), i)) for i, h in enumerate(headers)]
...
def kv_line(style: Style, label: str, value: str) -> str:
    return f"  {style.gray(label)}  {value}"
